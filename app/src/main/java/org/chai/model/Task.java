...
public class Task implements BaseEntity {
    ...
    /** To-many relationship, resolved on first access (and after reset). Changes to to-many relations are not persisted, make changes to the target entity. */
    public List<MalariaDetail> getMalariadetails() {
        if (malariadetails == null) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            MalariaDetailDao targetDao = daoSession.getMalariaDetailDao();
            List<MalariaDetail> malariadetailsNew = targetDao._queryTask_Malariadetails(uuid);
            synchronized (this) {
                if(malariadetails == null) {
                    malariadetails = malariadetailsNew;
                }
            }
        }
        return malariadetails;
    }

    /** Resets a to-many relationship, making the next get call to query for a fresh result. */
    public synchronized void resetMalariadetails() {
        malariadetails = null;
    }
